...
def _write_assembly_version_csharp(actions, label_name, dll_name, version):
    """Write a .cs file carrying the assembly version attributes.
    ...
    Returns:
      A File object for a generated .cs file, or None when there is no version.
    """
    if not version:
        return None
    ...
    actions.write(output, "\n".join([
        "// Generated by rules_dotnet from the target's `version` attribute.",
        "[assembly: System.Reflection.AssemblyVersion(\"%s\")]" % numeric,
        "[assembly: System.Reflection.AssemblyFileVersion(\"%s\")]" % numeric,
        "[assembly: System.Reflection.AssemblyInformationalVersion(\"%s\")]" % version,
        "",
    ]))
...
def AssemblyAction(
    ...
    assembly_version_cs = _write_assembly_version_csharp(actions, target_name, assembly_name, version)
    if assembly_version_cs:
        srcs = srcs + [assembly_version_cs]
