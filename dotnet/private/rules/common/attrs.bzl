...
CSHARP_COMMON_ATTRS = dicts.add(
    COMMON_ATTRS,
    {
        ...
        "excluded_analyzers": attr.string_list(
            doc = "A list of analyzer/source-generator assembly file names (e.g. \"Microsoft.Extensions.Configuration.Binder.SourceGeneration.dll\") to exclude from this compilation. Use this to turn off a specific bundled analyzer or source generator that is otherwise applied automatically via a NuGet dependency. Matching is by assembly file name (basename), and an entry that does not match any applied analyzer is silently ignored. The set of applied analyzers can differ per target framework (framework SDKs ship their own generators), so to exclude an analyzer only for specific frameworks wrap this in a `select()` on the `@rules_dotnet//dotnet:tfm_<framework>` config settings.",
            mandatory = False,
            default = [],
        ),
    },
