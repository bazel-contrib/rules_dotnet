...
load("//dotnet/private:providers.bzl", "DotnetAssemblyCompileInfo", "DotnetAssemblyRuntimeInfo", "DotnetTargetingPackInfo")
...
_COMMON_ATTRS = {
    ...
    "generate_assembly_info": attr.bool(
        doc = "Generate SDK-style assembly version, configuration, and target-framework attributes. Disable when supplying these attributes in source.",
        default = True,
    ),
    "assembly_metadata": attr.string_dict(
        doc = "AssemblyMetadata attributes, such as IsTrimmable=True for libraries that support trimming.",
    ),
...
STAMP_TOOL_ATTRS = {
    "_stamp2info": attr.label(
        default = "//tools/stamp2info",
        executable = True,
        cfg = "exec",
    ),
}

COMMON_ATTRS = dicts.add(_COMMON_ATTRS, STAMP_ATTRS, STAMP_TOOL_ATTRS)
...
CSHARP_GLOBAL_USINGS_ATTRS = {
    "implicit_usings": attr.bool(
        doc = "Generate the standard Microsoft.NET.Sdk implicit C# global usings.",
        default = False,
    ),
    "global_usings": attr.string_list(
        doc = "C# global usings generated for this target. Entries may include 'static ' or aliases.",
    ),
}
...
CSHARP_BOOTSTRAP_COMMON_ATTRS = dicts.add(
    _COMMON_ATTRS,
    CSHARP_LANGUAGE_ATTRS,
)
...
CSHARP_BOOTSTRAP_BINARY_COMMON_ATTRS = dicts.add(
    CSHARP_BOOTSTRAP_COMMON_ATTRS,
    BINARY_COMMON_ATTRS,
)
